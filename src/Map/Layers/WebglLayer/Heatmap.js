...
class Heatmap extends BaseLayer {
    ...
    init() {
        ...
        const effect1 = new THREE.ShaderPass(GaussianPass)
        effect1.uniforms.uResolution.value = [this.width, this.height]
    ...
    render(data = mockData, style = mockStyle) {
        ...
        if (material) {
            ...
            if (style.radius) material.uniforms.uRadius.value = style.radius
            if (style.blur) material.uniforms.uBlurFactor.value = style.blur
            if (this.camera)
                material.uniforms.uScale.value =
                    this.map.scale / this.camera.zoom
    ...
    genFS = (fs, sameValue = false) => {
        let res = fs
        if (sameValue) res = '#define SAME_VALUE\n' + res
